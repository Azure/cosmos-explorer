...
export const SearchableDropdown = <T,>({
    ...
    const filteredItems = items?.filter((item) =>
        getDisplayText(item).toLowerCase().includes(filterText.toLowerCase()),
    );
    ...
    const buttonLabel =
        selectedItem ? getDisplayText(selectedItem) : items?.length === 0 ? `No ${label}s Found` : placeholder;

    return (
        <div>
            <Label>{label}</Label>
            <button ref={buttonRef} className={className} onClick={handleButtonClick} style={buttonStyles} disabled={disabled}>
