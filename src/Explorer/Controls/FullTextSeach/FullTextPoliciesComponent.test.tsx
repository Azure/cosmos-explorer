...
describe("FullTextPoliciesComponent", () => {
  ...
  it("inherits the current and future default filters after resetting a path override", () => {
    const changed = jest.fn();
    const original: FullTextPolicy = {
      ...policy,
      fullTextPaths: [{ path: "/text", futurePathSetting: { preserved: true } }],
    };
    render(
      <FullTextPoliciesComponent
        fullTextPolicy={original}
        onFullTextPathChange={changed}
        allowStopwordCustomization
        isEditing
      />,
    ...
    expect(pathSettings.getByRole("checkbox", { name: "Enable stopword filtering" })).toBeChecked();
    fireEvent.click(pathSettings.getByRole("checkbox", { name: "Enable stopword filtering" }));
    expect(changed.mock.calls[changed.mock.calls.length - 1][0].fullTextPaths[0].filters).toEqual(["lowercase"]);
    fireEvent.click(inherit);
    expect(changed.mock.calls[changed.mock.calls.length - 1][0].fullTextPaths).toEqual(original.fullTextPaths);
    expect(screen.queryByRole("group", { name: "Stopwords for /text" })).not.toBeInTheDocument();
    ...
    fireEvent.click(inherit);
    expect(
      within(screen.getByRole("group", { name: "Stopwords for /text" })).getByRole("checkbox", {
        name: "Enable stopword filtering",
      }),
    ).not.toBeChecked();
    expect(original.defaultSpec?.filters).toEqual(["lowercase", "stop"]);
  });

  it("reports analysis-only path overrides instead of hiding them as inherited", () => {
    const changed = jest.fn();
    render(
      <FullTextPoliciesComponent
        fullTextPolicy={{ ...policy, fullTextPaths: [{ path: "/text", tokenizer: "word", filters: ["lowercase"] }] }}
        onFullTextPathChange={changed}
        allowStopwordCustomization
        isEditing
      />,
    ...
    expect(screen.getByRole("group", { name: "Stopwords for /text" })).toBeVisible();
    fireEvent.click(inherit);
    expect(changed.mock.calls[changed.mock.calls.length - 1][0].fullTextPaths).toEqual([{ path: "/text" }]);
  });

  it.each(["capability", "compatibility", "indexed"])(
    "preserves uneditable package/preset combinations without blocking saves: %s",
    (reason) => {
      if (reason === "capability") {
        updateUserContext({ databaseAccount: { ...account, properties: { capabilities: [] } } });
      }
      const persisted: FullTextPolicy = {
        ...policy,
        defaultSpec: { ...policy.defaultSpec, stopWordListKind: "extended" },
        fullTextPaths: [{ path: "/text" }],
      };
      const changed = jest.fn();
      const indexes = reason === "indexed" ? [{ path: "/text" }] : [];
      render(
        <FullTextPoliciesComponent
          fullTextPolicy={persisted}
          onFullTextPathChange={changed}
          allowStopwordCustomization={reason !== "compatibility"}
          isEditing
          fullTextIndexes={indexes}
        />,
      );
      expect(changed).toHaveBeenLastCalledWith(persisted, indexes, true);
      expect(screen.getByRole("combobox", { name: /^Default language/ })).toHaveAttribute("aria-disabled", "true");
      expect(persisted.defaultSpec?.stopWordListKind).toBe("extended");
    },
  );

  it("still blocks an editable invalid preset until the user corrects it", () => {
    const changed = jest.fn();
    render(
      <FullTextPoliciesComponent
        fullTextPolicy={{ ...policy, defaultSpec: { ...policy.defaultSpec, stopWordListKind: "extended" } }}
        onFullTextPathChange={changed}
        allowStopwordCustomization
        isEditing
      />,
    ...
    fireEvent.click(
      within(screen.getByRole("group", { name: "Default stopwords" })).getByRole("combobox", {
        name: "Stopword list",
      }),
    );
    fireEvent.click(screen.getByRole("option", { name: "Basic", exact: true }));
    expect(changed.mock.calls[changed.mock.calls.length - 1][2]).toBe(true);
  });

  it.each([{ tokenizer: "future" }, { filters: ["future"] }])(
    "preserves opaque path analysis without allowing an inheritance reset: %j",
    (analysis) => {
      const persisted: FullTextPolicy = {
        ...policy,
        fullTextPaths: [{ path: "/text", ...analysis, futureSetting: { preserved: true } }],
      };
      const changed = jest.fn();
      render(
        <FullTextPoliciesComponent
      ...
      expect(screen.getByRole("checkbox", { name: "Inherit the container's language and stopwords" })).toBeDisabled();
      expect(changed).toHaveBeenLastCalledWith(persisted, [], true);
    },
  );
