...
export const FullTextPoliciesComponent: React.FunctionComponent<FullTextPoliciesComponentProps> = ({
  ...
  React.useEffect(() => {
    callbacks.current.onFullTextPathChange(
      ...
      readOnly || isFullTextPolicyValid(policy, isEditing ? fullTextIndexes : emptyIndexes),
    );
    ...
    <CosmosFluentProvider className={styles.editor}>
      <Stack tokens={{ childrenGap: 4 }}>
              ...
              if (data.checked === true) {
                ...
                setPolicy((current) => {
                  ...
                  const next = {
                    ...
                    fullTextPaths: current.fullTextPaths.map((path) => ({
                      ...inheritFullTextPathAnalysis(path),
                      language: path.language ?? language,
                    })),
                  };
                  delete next.package;
        ...
        {policy.fullTextPaths.map((path, index) => {
          ...
          const inherited = isFullTextPathInherited(path);
          const pathError = !path.path.trim()
            ...
            : undefined;
          const needsAttention = !pathLocked && (pathError || getStopwordValidationError(path, policy.package));
                  ...
                  <Stack className={styles.pathFields} tokens={{ childrenGap: 4 }}>
                    ...
                    {showStopwords && !inherited && (
                      <StopwordSettings
                        ...
                        language={path.language ?? defaultLanguage}
                        packageName={policy.package}
                        spec={path}
