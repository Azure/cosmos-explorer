...
describe("full-text policy helpers", () => {
  ...
  describe("complete supported language and stopword policy matrix", () => {
    ...
    it.each(fullTextLanguages)(
      "$value removes all supported analysis overrides when inheriting",
      ({ value: language }) => {
        const path = {
          path: "/text",
          language,
  ...
  it("resets all supported analysis overrides while preserving unknown fields", () => {
    const path = {
      path: "/text",
      language: "fr-FR",
      stopWordListKind: "none",
      ...
      removeStopWords: ["the"],
      tokenizer: "word",
      filters: ["lowercase", "ascii", "stem", "stop"],
  ...
  it("treats a path as inherited only when every supported analysis field is absent", () => {
    expect(isFullTextPathInherited({ path: "/text", futureSetting: { preserved: true } })).toBe(true);
    const overrides: FullTextAnalysisSpec[] = [
      { language: "en-US" },
      { stopWordListKind: "none" },
      { addStopWords: [] },
      { removeStopWords: [] },
    ...
    for (const analysis of overrides) {
      expect(isFullTextPathInherited({ path: "/text", ...analysis })).toBe(false);
    }
  });
  ...
  it("ignores only locked analysis while continuing to validate editable paths", () => {
    const persisted: FullTextPolicy = {
      ...policy,
      defaultSpec: { ...policy.defaultSpec, stopWordListKind: "extended" },
      fullTextPaths: [{ path: "/text", language: "en-US", stopWordListKind: "extended" }],
    };
    expect(isFullTextPolicyValid(persisted)).toBe(false);
    expect(isFullTextPolicyValid(persisted, [{ path: "/text" }])).toBe(true);
    expect(
      isFullTextPolicyValid({ ...persisted, fullTextPaths: [...persisted.fullTextPaths, { path: "/other" }] }, [
        { path: "/text" },
      ]),
    ).toBe(true);
    expect(
      isFullTextPolicyValid({ ...persisted, fullTextPaths: [...persisted.fullTextPaths, { path: "" }] }, [
        { path: "/text" },
      ]),
    ).toBe(false);
    expect(
      isFullTextPolicyValid(
        {
          ...persisted,
          fullTextPaths: [
            ...persisted.fullTextPaths,
            { path: "/other", language: "en-US", stopWordListKind: "basic", addStopWords: ["two words"] },
          ],
        },
        [{ path: "/text" }],
      ),
    ...
    expect(
      isFullTextPolicyValid({ ...persisted, fullTextPaths: [...persisted.fullTextPaths, { path: "/text" }] }, [
        { path: "/text" },
      ]),
    ).toBe(false);
  });
