import { FullTextAnalysisSpec, FullTextIndex, FullTextPath, FullTextPolicy } from "Contracts/DataModels";
...
export const isFullTextPolicyValid = (
  policy: FullTextPolicy,
  lockedIndexes: readonly FullTextIndex[] = [],
): boolean => {
  ...
  const lockedPaths = new Set(lockedIndexes.map(({ path }) => path));
  const paths = policy.fullTextPaths.map(({ path }) => path);
  return (
    paths.every((path) => path.trim().length > 0) &&
    new Set(paths).size === paths.length &&
    (lockedPaths.size > 0 || getStopwordValidationError(policy.defaultSpec, policy.package) === undefined) &&
    policy.fullTextPaths.every(
      (path) => lockedPaths.has(path.path) || getStopwordValidationError(path, policy.package) === undefined,
    )
  );
};
...
export const isFullTextPathInherited = (path: FullTextPath): boolean =>
  analysisFields.every((field) => path[field] === undefined);

export const inheritFullTextPathAnalysis = (path: FullTextPath): FullTextPath => {
  const result = { ...path };
