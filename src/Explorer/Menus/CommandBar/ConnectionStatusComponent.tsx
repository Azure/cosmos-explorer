  ...
  React.useEffect(() => {
    ...
    if (isActive) {
      intervalId = setInterval(() => {
        ...
        let computedMilliSecond: any =
          String(milliSecondCounter).length === 1
            ? `00${milliSecondCounter}`
            : String(milliSecondCounter).length === 2
            ? `0${milliSecondCounter}`
            : milliSecondCounter;
        let computedSecond: any = String(secondCounter).length === 1 ? `0${secondCounter}` : secondCounter;
        let computedMinute: any = String(minuteCounter).length === 1 ? `0${minuteCounter}` : minuteCounter;
