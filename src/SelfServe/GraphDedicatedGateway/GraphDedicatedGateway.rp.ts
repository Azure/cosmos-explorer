...
import GraphDedicatedGateway from "./GraphDedicatedGateway";
import {
  FetchPricesResponse,
  RegionsResponse,
...
export const updateDedicatedGatewayResource = async (sku: string, instances: number): Promise<string> => {
  ...
  const telemetryData = { ...body, httpMethod: "PUT", selfServeClassName: GraphDedicatedGateway.name };
  ...
  try {
    ...
    const failureTelemetry = { ...body, e, selfServeClassName: GraphDedicatedGateway.name };
...
export const deleteDedicatedGatewayResource = async (): Promise<string> => {
  const path = getPath(userContext.subscriptionId, userContext.resourceGroup, userContext.databaseAccount.name);
  const telemetryData = { httpMethod: "DELETE", selfServeClassName: GraphDedicatedGateway.name };
  ...
  try {
    ...
    const failureTelemetry = { e, selfServeClassName: GraphDedicatedGateway.name };
...
export const getDedicatedGatewayResource = async (): Promise<GraphDedicatedGatewayServiceResource> => {
  const path = getPath(userContext.subscriptionId, userContext.resourceGroup, userContext.databaseAccount.name);
  const telemetryData = { httpMethod: "GET", selfServeClassName: GraphDedicatedGateway.name };
  const getResourceTimeStamp = selfServeTraceStart(telemetryData);
  let armRequestResult;
  try {
    armRequestResult = await armRequestWithoutPolling<GraphDedicatedGatewayServiceResource>({
      host: configContext.ARM_ENDPOINT,
      path,
      method: "GET",
      apiVersion,
...
export const getCurrentProvisioningState = async (): Promise<DedicatedGatewayResponse> => {
  try {
    ...
    return {
      ...
      endpoint: response.properties.GraphDedicatedGatewayEndPoint,
    };
  } catch (e) {
