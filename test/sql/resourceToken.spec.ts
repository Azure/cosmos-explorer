...
test("Resource token", async () => {
  ...
  const account = await armClient.databaseAccounts.get(resourceGroupName, "portal-sql-runner-wus");
  const keys = await armClient.databaseAccounts.listKeys(resourceGroupName, "portal-sql-runner-wus");
